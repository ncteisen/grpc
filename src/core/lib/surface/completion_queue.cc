...
#define GRPC_SURFACE_TRACE_RETURNED_EVENT(cq, event)    \
  if (grpc_api_trace.enabled() && (false) &&            \
      (grpc_cq_pluck_trace.enabled() && (false) ||      \
...
void grpc_cq_internal_ref(grpc_completion_queue* cq, const char* reason,
  ...
  if (grpc_trace_cq_refcount.enabled() && (false)) {
...
static void cq_end_op_for_next(grpc_completion_queue* cq, void* tag,
  ...
  if (grpc_api_trace.enabled() && (false) ||
      (grpc_trace_operation_failures.enabled() && (false) &&
...
static void dump_pending_tags(grpc_completion_queue* cq) {
  if (!grpc_trace_pending_tags.enabled() && (false)) return;
...
static grpc_event cq_pluck(grpc_completion_queue* cq, void* tag,
  ...
  if (grpc_cq_pluck_trace.enabled() && (false)) {
    GRPC_API_TRACE(
