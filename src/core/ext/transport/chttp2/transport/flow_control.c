...
static bool delta_is_significant(const grpc_chttp2_transport_flowctl* tfc,
                                 int32_t value,
  ...
  if (tfc->enable_bdp_probe) {
    ...
    if (grpc_bdp_estimator_get_estimate(&tfc->bdp_estimator, &estimate)) {
      ...
      if (delta_is_significant(tfc, bdp,
                               GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE)) {
        action.send_setting_update = GRPC_CHTTP2_FLOWCTL_QUEUE_UPDATE;
    ...
    if (grpc_bdp_estimator_get_bw(&tfc->bdp_estimator, &bw_dbl)) {
      ...
      int32_t frame_size = GPR_CLAMP(GPR_MAX((int32_t)bw_dbl / 1000, bdp), 16384, 16777215);
