...
static void posttrace(shadow_flow_control* shadow_fc,
  ...
  uint32_t remote_window =
      tfc->t->settings[GRPC_PEER_SETTINGS]
                      [GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE];
  char* trw_str = fmt_int64_diff_str(shadow_fc->remote_window, tfc->remote_window);
  char* tlw_str = fmt_int64_diff_str(shadow_fc->target_window,
                          grpc_chttp2_target_announced_window(tfc));
  char* taw_str = fmt_int64_diff_str(shadow_fc->announced_window, tfc->announced_window);
  char* srw_str;
  char* slw_str;
  char* saw_str;
  if (sfc != NULL) {
    ...
    slw_str = fmt_int64_diff_str(shadow_fc->local_window_delta + acked_local_window,
                      sfc->local_window_delta + acked_local_window);
    saw_str = fmt_int64_diff_str(shadow_fc->announced_window_delta + acked_local_window,
...
static void trace_action(grpc_chttp2_transport_flowctl* tfc, grpc_chttp2_flowctl_action action) {
  char* iw_str = fmt_uint32_diff_str(tfc->t->settings[GRPC_SENT_SETTINGS][GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE], action.initial_window_size);
  char* mf_str = fmt_uint32_diff_str(tfc->t->settings[GRPC_SENT_SETTINGS][GRPC_CHTTP2_SETTINGS_MAX_FRAME_SIZE], action.max_frame_size);
  gpr_log(GPR_DEBUG, "t[%s],  s[%s], settings[%s] iw:%s mf:%s",
          urgency_to_string(action.send_transport_update),
          urgency_to_string(action.send_stream_update), urgency_to_string(action.send_setting_update), iw_str, mf_str);
  gpr_free(iw_str);
  gpr_free(mf_str);
}
...
#define TRACEACTION(tfc, action) GRPC_FLOW_CONTROL_IF_TRACING(trace_action(tfc, action))
#else
#define PRETRACE(tfc, sfc)
#define POSTTRACE(tfc, sfc, reason)
...
static bool delta_is_significant(const grpc_chttp2_transport_flowctl* tfc, int32_t value,
                                 grpc_chttp2_setting_id setting_id) {
  int64_t delta =
      (int64_t)value - (int64_t)tfc->t->settings[GRPC_LOCAL_SETTINGS][setting_id];
...
static double get_target_under_memory_pressure(grpc_chttp2_transport_flowctl* tfc,
                                               double target) {
...
grpc_chttp2_flowctl_action grpc_chttp2_flowctl_get_action(
    grpc_chttp2_transport_flowctl* tfc,
    grpc_chttp2_stream_flowctl* sfc, bool read_path) {
  ...
  if (tfc->enable_bdp_probe) {
    ...
    if (read_path) {
      ...
      if (grpc_bdp_estimator_get_estimate(&tfc->bdp_estimator, &estimate)) {
        ...
        if (delta_is_significant(tfc, bdp,
                                 GRPC_CHTTP2_SETTINGS_INITIAL_WINDOW_SIZE)) {
          action.send_setting_update =
      ...
      if (grpc_bdp_estimator_get_bw(&tfc->bdp_estimator, &bw_dbl)) {
        ...
        if (delta_is_significant(tfc, frame_size,
